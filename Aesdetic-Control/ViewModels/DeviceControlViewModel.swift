...
class DeviceControlViewModel: ObservableObject {
    ...
    @Published private(set) var presetStoreContentHealthByDeviceId: [String: PresetStoreContentHealth] = [:]
    @Published private(set) var presetStoreTransactionStatusByDeviceId: [String: PresetStoreTransactionStatus] = [:]
    ...
    private var isProvisioningNetworkTransitionActive = false
    ...
    @Published private(set) var alexaIntegrationEnabledByDeviceId: [String: Bool] = [:]
    @Published private(set) var alexaMirrorSyncInProgressByDeviceId: Set<String> = []
    @Published private(set) var alexaMirrorConflictSlotsByDeviceId: [String: Set<Int>] = [:]
    ...
    // Temporary/live transitions must stay state-only (`tt`/segmented state writes).
    // Persistent transition saves (+Preset) and automations are the only transition
    // paths that write playlist/preset records to presets.json.
    ...
    private let firstDiscoveryTimeSyncKeyPrefix = "aesdetic_first_discovery_time_sync_v2."
    ...
    private func plannedTransitionProfile(
        ...
        let storageSlotLimit = context == .persistentAutomation ? available : nil
        ...
        if let storageSlotLimit {
            fitsStorage = chosenSlots <= storageSlotLimit
            if !fitsStorage {
                for candidate in candidates.dropFirst() {
                    ...
                    chosenSteps = steps
                    chosenSlots = slots
                    fitsStorage = slots <= storageSlotLimit
                    if fitsStorage {
                        break
                    }
                }
        ...
        return TransitionStepProfile(
            ...
            maxDurationSecondsAtCurrentQuality: storageSlotLimit.map { maxDurationSeconds(forSlots: $0, legSeconds: chosenLeg) }
        )
    }
    ...
    func debugClearPresetStoreHealthForTests(deviceId: String) {
        ...
        presetStoreContentHealthByDeviceId.removeValue(forKey: deviceId)
        presetStoreWritePauseUntilByDeviceId.removeValue(forKey: deviceId)
        lastPresetStoreHealthMessageByDeviceId.removeValue(forKey: deviceId)
        lastPresetStoreHealthEventByDeviceId.removeValue(forKey: deviceId)
    ...
    func debugSetPresetCacheForTests(deviceId: String, presets: [WLEDPreset]?) {
        if let presets {
            presetsCache[deviceId] = presets
        } else {
            presetsCache.removeValue(forKey: deviceId)
            presetSlotStatus.removeValue(forKey: deviceId)
        }
    }
    #endif
    ...
    func runAutomationTransition(
        ...
        let usePlaylistForLongTransition = shouldPersistPlaylist
    ...
    private let brightnessPreviewCadence = BrightnessInteractionPolicy.previewCadence
    private struct BrightnessInteractionSession {
        let id: UUID
        let requiresAtomicCommit: Bool
        var lastPreviewValue: Int?
    }
    private var brightnessInteractionSessions: [String: BrightnessInteractionSession] = [:]
    private var pendingBrightnessPreviewValues: [String: Int] = [:]
    private var brightnessPreviewTasks: [String: Task<Void, Never>] = [:]
    private var brightnessRunCancellationTasks: [String: Task<Void, Never>] = [:]
    private var lastBrightnessPreviewDispatchAt: [String: Date] = [:]
    ...
    func previewManualColorChange(
        ...
        guard isRealTimeEnabled, webSocketManager.isDeviceConnected(device.id) else {
            return
        }
        ...
        let targets = previewManualColorTargets(for: device, segmentId: segmentId)
        if let temperature,
           temperatureStopsUseCCT(for: device),
           supportsCCTOutput(for: device, segmentId: segmentId) {
            let cct = segmentUsesKelvinCCT(for: device, segmentId: segmentId)
                ? kelvinValue(for: device, normalized: temperature)
                : Segment.eightBitValue(fromNormalized: temperature)
            let clearRGB = rgbArrayForTemperature(temperature)
            let clearUpdates = targets.map { target in
                var clearColor = clearRGB
                if supportsWhite(for: device, segmentId: target.id) {
                    clearColor.append(0)
                }
            ...
            webSocketManager.sendStateUpdate(
                WLEDStateUpdate(on: true, seg: clearUpdates, transitionDeciseconds: 1),
                to: device.id,
                debugLogging: false
            )

            let cctUpdates = targets.map { target in
                SegmentUpdate(
                    id: target.id,
                    start: target.start,
                    stop: target.stop,
            ...
            webSocketManager.sendStateUpdate(
                WLEDStateUpdate(on: true, seg: cctUpdates, transitionDeciseconds: 1),
                to: device.id,
                debugLogging: false
            )
        } else {
            let updates = targets.map { target in
                var colorArray = [rgb[0], rgb[1], rgb[2]]
                if let whiteLevel {
                    colorArray.append(Int(round(max(0.0, min(1.0, whiteLevel)) * 255.0)))
                } else if supportsWhite(for: device, segmentId: target.id) {
                    colorArray.append(0)
                }
                return SegmentUpdate(
                    id: target.id,
            ...
            webSocketManager.sendStateUpdate(
                WLEDStateUpdate(on: true, seg: updates, transitionDeciseconds: 1),
                to: device.id,
                debugLogging: false
            )
        }
    ...
    private func previewManualColorTargets(
        for device: WLEDDevice,
        segmentId: Int
    ) -> [(id: Int, start: Int?, stop: Int?)] {
        if usesManualSegmentation(for: device.id),
           ...
           !segments.isEmpty {
            return segments
                ...
                .map { index, segment in
                    (id: segment.id ?? index, start: nil, stop: nil)
                }
        }

        let ledCount = totalLEDCount(for: device)
        if shouldUseAppManagedSegments(for: device, ledCount: ledCount) {
            let count = segmentCount(for: device, ledCount: ledCount)
            let ranges = segmentStops(totalLEDs: ledCount, segmentCount: count)
            if !ranges.isEmpty {
                let layout = ranges.map { SegmentBounds(start: $0.start, stop: $0.stop) }
                let layoutIsCurrent = appManagedSegmentLayouts[device.id] == layout
                    && segmentsMatchLayout(device.state?.segments ?? [], layout: layout, segmentCount: count)
                return ranges.enumerated().map { index, range in
                    // Bounds are needed only to establish a layout. Omitting
                    // them from subsequent live writes keeps preview packets
                    // compact and avoids repeatedly rebuilding WLED segments.
                    (id: index,
                     start: layoutIsCurrent ? nil : range.start,
                     stop: layoutIsCurrent ? nil : range.stop)
                }
            }
        }

        if let segments = device.state?.segments, segments.count > 1 {
            return segments.enumerated().map { index, segment in
                (id: segment.id ?? index, start: nil, stop: nil)
            }
        }

        return [(id: segmentId, start: nil, stop: nil)]
    }
    ...
    private init() {
        ...
        if AppRuntimeEnvironment.isRunningUITests {
            ...
            devices = [Self.settingsUITestDevice]
            return
        }
    ...
    private static var settingsUITestDevice: WLEDDevice {
        WLEDDevice(
            id: "020000000001",
            name: "UI Test Device",
            ipAddress: "192.0.2.1",
            isOnline: false,
            brightness: 128,
            currentColor: Color(red: 1.0, green: 0.63, blue: 0.20),
            productType: .generic,
            setupState: .genericManual,
            location: .bedroom,
            state: WLEDState(
                brightness: 128,
                isOn: true,
    ...
    private func applyPersistedDevices(_ persistedDevices: [WLEDDevice], replaceExisting: Bool, source: String) {
        let reconciledPersistedDevices = WLEDDeviceIdentity.reconciledDevices(persistedDevices)
        let placeholders = reconciledPersistedDevices.filter { isPlaceholderDevice($0) }
        let persisted = reconciledPersistedDevices.filter { !isPlaceholderDevice($0) }
        ...
        if replaceExisting || devices.isEmpty {
            ...
            let existingIdentities = Set(devices.map {
                WLEDDeviceIdentity.deduplicationKey(for: $0.id)
            })
            devices = WLEDDeviceIdentity.reconciledDevices(devices + persisted)
            newlyAdded = devices.filter {
                !existingIdentities.contains(WLEDDeviceIdentity.deduplicationKey(for: $0.id))
            }
        }
    ...
    private func handleDiscoveredDevices(_ discoveredDevices: [WLEDDevice]) async {
        ...
        for rawDiscoveredDevice in discoveredDevices {
            var discoveredDevice = WLEDDeviceIdentity.normalized(rawDiscoveredDevice)
            ...
            if let existingIndex = devices.firstIndex(where: {
                WLEDDeviceIdentity.matches($0.id, discoveredDevice.id)
            }) {
                // Update existing device with any new information
                let previousDevice = devices[existingIndex]
                let previousId = previousDevice.id
                let previousOnline = previousDevice.isOnline
                let previousName = previousDevice.name
                let updatedDevice = deviceByMergingDiscovery(
                    discoveredDevice,
                    preservingMetadataFrom: previousDevice
                )
                if previousId != updatedDevice.id {
                    migrateCanonicalDeviceId(from: previousId, to: updatedDevice.id)
                    await coreDataManager.deleteDevice(id: previousId)
                }
                ...
                discoveredDevice = updatedDevice
            ...
            } else if let existingIndex = devices.firstIndex(where: {
                $0.ipAddress == discoveredDevice.ipAddress && !isPlaceholderDevice($0)
            }) {
                // Same physical device, but discovery has now resolved a different canonical ID.
                // Preserve setup/profile metadata so an already configured card does not regress to setup-required.
                let previousDevice = devices[existingIndex]
                let previousId = previousDevice.id
                let previousOnline = previousDevice.isOnline
                let previousName = previousDevice.name
                let updatedDevice = deviceByMergingDiscovery(discoveredDevice, preservingMetadataFrom: previousDevice)
    ...
    private func scheduleFirstDiscoveryTimeSyncIfNeeded(for device: WLEDDevice) {
        ...
        Task { [weak self] in
            guard let self else { return }
            // First discovery should not interrupt launch with a permission prompt.
            // Setup and solar editors request location when the user enters that flow.
            let coordinate = await AutomationStore.shared.currentCoordinate(requestAuthorization: false)
    ...
    private func deviceByMergingDiscovery(
        _ discoveredDevice: WLEDDevice,
        preservingMetadataFrom existingDevice: WLEDDevice
    ) -> WLEDDevice {
        let resolvedName = shouldApplyNameUpdate(
            existingName: existingDevice.name,
            candidateName: discoveredDevice.name
        ) ? discoveredDevice.name : existingDevice.name
        ...
        return WLEDDevice(
            ...
            currentColor: discoveredDevice.currentColor,
            temperature: discoveredDevice.temperature ?? existingDevice.temperature,
            autoWhiteMode: discoveredDevice.autoWhiteMode ?? existingDevice.autoWhiteMode,
            productType: existingDevice.productType,
            setupState: existingDevice.setupState,
            profileId: existingDevice.profileId,
            lookId: existingDevice.lookId,
            profileVersionApplied: existingDevice.profileVersionApplied,
            managedPresetIds: existingDevice.managedPresetIds,
            backupSnapshotId: existingDevice.backupSnapshotId,
            lastProfileAppliedAt: existingDevice.lastProfileAppliedAt,
            location: existingDevice.location,
            lastSeen: Date(),
            state: discoveredDevice.state ?? existingDevice.state,
            presetNamesById: discoveredDevice.presetNamesById ?? existingDevice.presetNamesById,
            playlistNamesById: discoveredDevice.playlistNamesById ?? existingDevice.playlistNamesById
        )
    }

    private func migrateCanonicalDeviceId(from oldDeviceId: String, to newDeviceId: String) {
        guard oldDeviceId != newDeviceId else { return }

        if activeDeviceId == oldDeviceId {
            activeDeviceId = newDeviceId
        }
        if selectedDevices.remove(oldDeviceId) != nil {
            selectedDevices.insert(newDeviceId)
        }
        if let optimisticState = uiToggleStates[oldDeviceId] {
            uiToggleStates[newDeviceId] = optimisticState
            uiToggleStates.removeValue(forKey: oldDeviceId)
        }
        if let pending = pendingToggles[oldDeviceId] {
            pendingToggles[newDeviceId] = pending
            pendingToggles.removeValue(forKey: oldDeviceId)
        }
        if let timer = toggleTimers[oldDeviceId] {
            toggleTimers[newDeviceId] = timer
            toggleTimers.removeValue(forKey: oldDeviceId)
        }
        if let userInputAt = lastUserInput[oldDeviceId] {
            lastUserInput[newDeviceId] = userInputAt
            lastUserInput.removeValue(forKey: oldDeviceId)
        }
        if let recentWrite = recentControlWriteSuccessAtByDeviceId[oldDeviceId] {
            recentControlWriteSuccessAtByDeviceId[newDeviceId] = recentWrite
            recentControlWriteSuccessAtByDeviceId.removeValue(forKey: oldDeviceId)
        }
        if let task = pendingDiscoveryStateRefreshTasks[oldDeviceId] {
            task.cancel()
            pendingDiscoveryStateRefreshTasks.removeValue(forKey: oldDeviceId)
        }
        if let stops = latestGradientStops[oldDeviceId] {
            latestGradientStops[newDeviceId] = stops
            latestGradientStops.removeValue(forKey: oldDeviceId)
        }
        if hasLiveGradientHydration.remove(oldDeviceId) != nil {
            hasLiveGradientHydration.insert(newDeviceId)
        }
        if appManagedSegmentDevices.remove(oldDeviceId) != nil {
            appManagedSegmentDevices.insert(newDeviceId)
        }
    }
    ...
    func removeDevice(_ device: WLEDDevice) async {
        ...
        cancelBrightnessInteraction(for: device.id)
        savedTransitionDefaults.removeValue(forKey: device.id)
        savedTransitionDefaultRunIds.removeValue(forKey: device.id)
    ...
    func beginBrightnessInteraction(_ device: WLEDDevice) {
        guard brightnessInteractionSessions[device.id] == nil else { return }

        let actualDevice = devices.first(where: { $0.id == device.id }) ?? device
        brightnessInteractionSessions[device.id] = BrightnessInteractionSession(
            id: UUID(),
            requiresAtomicCommit: !BrightnessInteractionPolicy.permitsPreview(
                isOn: actualDevice.isOn,
                currentBrightness: actualDevice.brightness,
                requestedBrightness: max(1, actualDevice.brightness)
            ),
        ...
        let cancellationTask = Task { @MainActor [weak self] in
            guard let self, self.hasKnownActiveRun(for: device.id) else { return }
            await self.cancelActiveRun(
                for: device,
                force: true,
                endReason: .cancelledByManualInput
            )
        }
        brightnessRunCancellationTasks[device.id] = cancellationTask
    }

    func previewBrightnessInteraction(_ device: WLEDDevice, brightness: Int) {
        if brightnessInteractionSessions[device.id] == nil {
            beginBrightnessInteraction(device)
        }
        guard let session = brightnessInteractionSessions[device.id],
              !session.requiresAtomicCommit else {
            return
        }

        let clamped = max(0, min(255, brightness))
        // Power-off and off-to-on restoration remain release-only because they
        // preserve remembered brightness and may need to restore a gradient.
        guard clamped > 0 else { return }
        pendingBrightnessPreviewValues[device.id] = clamped
        guard brightnessPreviewTasks[device.id] == nil else { return }

        let sessionId = session.id
        brightnessPreviewTasks[device.id] = Task<Void, Never> { @MainActor [weak self] in
            guard let self else { return }
            await self.runBrightnessPreviewLoop(
                for: device,
                sessionId: sessionId
            )
        }
    ...
    func commitBrightnessInteraction(_ device: WLEDDevice, brightness: Int) async {
        let hadInteraction = brightnessInteractionSessions[device.id] != nil
        let previewTask = brightnessPreviewTasks.removeValue(forKey: device.id)
        previewTask?.cancel()
        pendingBrightnessPreviewValues.removeValue(forKey: device.id)
        // A preview can already be inside URLSession or WebSocket dispatch when
        // the thumb is released. Await its settlement before the authoritative
        // commit so a late preview can never overwrite the exact final value.
        if let previewTask {
            await previewTask.value
        }
        if let cancellationTask = brightnessRunCancellationTasks.removeValue(forKey: device.id) {
            await cancellationTask.value
        }
        brightnessInteractionSessions.removeValue(forKey: device.id)
        lastBrightnessPreviewDispatchAt.removeValue(forKey: device.id)

        await updateDeviceBrightness(
            device,
            brightness: max(0, min(255, brightness)),
            userInitiated: !hadInteraction,
            origin: .user,
            forceWrite: hadInteraction
        )
    }

    func cancelBrightnessInteraction(_ device: WLEDDevice) {
        cancelBrightnessInteraction(for: device.id)
    }

    private func cancelBrightnessInteraction(for deviceId: String) {
        brightnessPreviewTasks.removeValue(forKey: deviceId)?.cancel()
        pendingBrightnessPreviewValues.removeValue(forKey: deviceId)
        brightnessInteractionSessions.removeValue(forKey: deviceId)
        lastBrightnessPreviewDispatchAt.removeValue(forKey: deviceId)
        brightnessRunCancellationTasks.removeValue(forKey: deviceId)?.cancel()
    }

    private func runBrightnessPreviewLoop(
        for device: WLEDDevice,
        sessionId: UUID
    ) async {
        if let cancellationTask = brightnessRunCancellationTasks[device.id] {
            await cancellationTask.value
        }

        while !Task.isCancelled,
              brightnessInteractionSessions[device.id]?.id == sessionId,
              let value = pendingBrightnessPreviewValues.removeValue(forKey: device.id) {
            if let lastDispatch = lastBrightnessPreviewDispatchAt[device.id] {
                let remaining = brightnessPreviewCadence - Date().timeIntervalSince(lastDispatch)
                if remaining > 0 {
            ...
            guard !Task.isCancelled,
                  brightnessInteractionSessions[device.id]?.id == sessionId else {
                break
            }

            lastBrightnessPreviewDispatchAt[device.id] = Date()
            let sent = await sendBrightnessPreview(value, to: device)
            if sent,
               var session = brightnessInteractionSessions[device.id],
               session.id == sessionId {
                session.lastPreviewValue = value
                brightnessInteractionSessions[device.id] = session
            }
        }

        if brightnessInteractionSessions[device.id]?.id == sessionId {
            brightnessPreviewTasks.removeValue(forKey: device.id)
            // A value may arrive between the loop's final lookup and task
            // cleanup. Start one replacement worker so the latest value wins.
            if pendingBrightnessPreviewValues[device.id] != nil {
                previewBrightnessInteraction(
                    device,
                    brightness: pendingBrightnessPreviewValues[device.id] ?? device.brightness
                )
            }
        }
    }

    private func sendBrightnessPreview(_ brightness: Int, to device: WLEDDevice) async -> Bool {
        let update = WLEDStateUpdate(
            on: true,
            bri: max(1, min(255, brightness)),
            transitionDeciseconds: 1,
            lor: 0
        )

        if isRealTimeEnabled, webSocketManager.isDeviceConnected(device.id) {
            let dispatched = await webSocketManager.sendStateUpdateAwaitingDispatch(
                update,
                to: device.id,
                timeout: 0.35,
                debugLogging: false
        ...
        do {
            _ = try await apiService.updateState(for: device, state: update)
            return true
        } catch {
            #if DEBUG
            print("brightness.preview.failed device=\(device.id) error=\(error.localizedDescription)")
            #endif
            return false
        }
    }

    func updateDeviceBrightness(
        _ device: WLEDDevice,
        brightness: Int,
        ...
        if !forceWrite,
           actualDevice.brightness == brightness,
           actualDevice.isOn == expectedPowerState {
            return
        }
    ...
    func updateAnimationBrightness(_ device: WLEDDevice, brightness: Int) async {
        ...
        if wsConnected {
            ...
            #if DEBUG
            print("⚠️ Animation brightness WebSocket dispatch failed for \(device.name); falling back to HTTP")
            #endif
        }
    ...
    func resumeRealTimeConnectionsIfNeeded() {
        guard !isProvisioningNetworkTransitionActive else { return }
        guard isRealTimeEnabled else { return }
    ...
    func setProvisioningNetworkTransitionActive(_ isActive: Bool) {
        guard isProvisioningNetworkTransitionActive != isActive else { return }
        isProvisioningNetworkTransitionActive = isActive
    ...
    func presentPresetStoreRepairGuidance(for deviceId: String) {
        let deviceName = devices.first(where: { $0.id == deviceId })?.name ?? "This lamp"
        let detail = lastPresetStoreHealthMessageByDeviceId[deviceId]
            .map { " Details: \($0)" }
            ?? ""
        presentError(
            .apiError(
                message: "\(deviceName)'s saved-item store needs repair. Keep the lamp powered, open its WLED page, and restore a verified presets.json backup before retrying.\(detail)"
            )
        )
    }
    ...
    private func mapToWLEDError(_ error: Error, device: WLEDDevice?) -> WLEDError {
        if let apiError = error as? WLEDAPIError {
            ...
            case .presetStoreInsufficientSpace(let detail):
                let resolved = resolvedDeviceName(for: device, providedName: nil) ?? "This lamp"
                return .apiError(message: "\(resolved) storage is too full to save. Delete saved transitions, automations, or WLED presets to make room, then try again. \(detail)")
            }
        }
        if let urlError = error as? URLError {
            switch urlError.code {
    ...
    private func presentPresetStoreCapacityError(
        device: WLEDDevice,
        detail: String
    ) {
        presentError(.apiError(message: "\(device.name) storage is too full to save. \(detail)"))
    }

    private func storageDurationDescription(_ seconds: Double) -> String {
        ...
        if clamped >= 60 {
            let minutes = Int(clamped.rounded() / 60)
    ...
    func applyGradientStopsAcrossStrip(_ device: WLEDDevice, stops: [GradientStop], ledCount: Int, stopTemperatures: [UUID: Double]? = nil, stopWhiteLevels: [UUID: Double]? = nil, disableActiveEffect: Bool = false, segmentId: Int = 0, interpolation: GradientInterpolation = .linear, brightness: Int? = nil, on: Bool? = nil, transitionDurationSeconds: Double? = nil, forceNoPerCallTransition: Bool = false, releaseRealtimeOverride: Bool = true, userInitiated: Bool = true, origin: SyncOrigin = .user, preferSegmented: Bool = false, forceSegmentedOnly: Bool = false) async {
        ...
        let solidTemperatureValues = stopTemperatures.map { map in sortedStops.compactMap { map[$0.id] } } ?? []
        let solidWhiteValues = stopWhiteLevels.map { map in sortedStops.compactMap { map[$0.id] } } ?? []
        let solidHasUniformTemperature = solidTemperatureValues.isEmpty ||
            (solidTemperatureValues.count == sortedStops.count &&
             solidTemperatureValues.allSatisfy { abs($0 - solidTemperatureValues[0]) < 0.001 })
        let solidHasUniformWhite = solidWhiteValues.isEmpty ||
            (solidWhiteValues.count == sortedStops.count &&
             solidWhiteValues.allSatisfy { abs($0 - solidWhiteValues[0]) < 0.001 })
        let canUseCompactSolidUpdate = isSolidColor && solidHasUniformTemperature && solidHasUniformWhite && !willUseSegmented
        ...
        if isSolidColor, willUseSegmented, !canUseCompactSolidUpdate {
    ...
    private func segmentColors(for gradient: LEDGradient, count: Int) -> [[Int]] {
        ...
        return (0..<count).map { idx in
            // Each WLED segment renders one flat color, so sampling its center
            // is the least-biased representation of the continuous UI rail.
            let t = (Double(idx) + 0.5) / Double(count)
            let color = GradientSampler.sampleColor(at: t, stops: sortedStops, interpolation: gradient.interpolation)
    ...
    private func segmentedPresetState(
        ...
        compactForPresetStore: Bool = false
    ) -> WLEDStateUpdate {
        let totalLEDs = totalLEDCount(for: device)
        ...
        return WLEDStateUpdate(
            ...
            bri: brightness,
            seg: updates,
            mainSegment: compactForPresetStore ? nil : device.state?.mainSegment
        )
    }
    ...
    private func availableIds(from maxId: Int, through minId: Int = appManagedPresetLowerBound, excluding used: Set<Int>, count: Int) -> [Int]? {
        guard count > 0 else { return [] }
        var results: [Int] = []
    ...
    private func availablePlaylistId(excluding used: Set<Int>, range: ClosedRange<Int> = appManagedPresetRange) -> Int? {
    ...
    private func shouldBlockPresetStoreMutation(deviceId: String) async -> Bool {
        if presetStoreTransactionStatusByDeviceId[deviceId]?.blocksPersistentEdits == true {
            return true
        }
    ...
    private func requireCommittedPresetStoreOutcome(
        _ outcome: PresetStoreMutationOutcome
    ) throws {
        guard outcome.isCommitted else {
            throw WLEDAPIError.presetStoreUnreadable(
                "The lamp recovered its preset store, but the requested edit was not committed."
            )
        }
    }
    ...
    private func savePresetWithRetry(
        ...
        _ = retryAttempts
        let outcome = try await apiService.savePreset(request, to: device)
        guard outcome.isCommitted else {
            throw WLEDAPIError.presetStoreUnreadable(
                "The preset transaction did not commit."
            )
        }
        try? await Task.sleep(nanoseconds: presetSaveDelayNanos)
    }
    ...
    private func savePlaylistWithRetry(
        ...
        _ = retryAttempts
        let outcome = try await apiService.savePlaylist(request, to: device)
        guard outcome.isCommitted else {
            throw WLEDAPIError.presetStoreUnreadable(
                "The playlist transaction did not commit."
            )
        }
        try? await Task.sleep(nanoseconds: playlistSaveDelayNanos)
    }
    ...
    func createTransitionPlaylist(
        ...
        guard persist else {
            #if DEBUG
            print("transition.playlist_build.skipped_live_preset_store_path_removed device=\(device.id)")
            #endif
            return nil
        }
        ...
        let effectiveOperationId = debugOperationId
            ...
            ?? self.debugOperationId(prefix: "playlist-persist")
        let operationContext = debugOperationContext(effectiveOperationId)
        print("🔎 Playlist build for \(device.name): mode=2-preset, storage=persistent, playlist=full-store-transaction, duration=\(String(format: "%.1f", durationSeconds))s\(operationContext)")
        #endif
        ...
        presetsCache[device.id] = existingPresets
        var usedPresetIds = Set(existingPresets.map { $0.id }.filter { (1...250).contains($0) })
        let context: TransitionGenerationContext = .persistentAutomation
        let persistentAllowedRange = persistentAutomationPresetRange
        let appManagedUsedPresetCount = existingPresets.filter { persistentAllowedRange.contains($0.id) }.count
        let stepProfile = plannedTransitionProfile(
            durationSec: durationSeconds,
            startGradient: from,
            endGradient: to,
            startBrightness: startBrightness,
            endBrightness: endBrightness,
            context: context,
            usedPresetCountOverride: appManagedUsedPresetCount,
            device: device
        )
        if persist, !stepProfile.fitsStorage {
            #if DEBUG
            let availableText = stepProfile.availableSlots.map(String.init) ?? "n/a"
            let maxDuration = stepProfile.maxDurationSecondsAtCurrentQuality ?? 0
            print("⚠️ Playlist creation blocked by preset-slot capacity for \(device.name): required=\(stepProfile.slotsRequired) available=\(availableText) reserve=\(presetSlotReserve) quality=\(stepProfile.qualityLabel.rawValue) maxDuration=\(String(format: "%.1f", maxDuration))s")
            #endif
            let capacityDuration = stepProfile.maxDurationSecondsAtCurrentQuality ?? 0
            let durationHint = capacityDuration > 0
                ? "Shorten this transition to \(storageDurationDescription(capacityDuration)) or delete saved transitions/automations to make room."
                : "Delete saved transitions or automations to make room."
            presentPresetStoreCapacityError(
                device: device,
                detail: "This transition needs \(stepProfile.slotsRequired) saved entries, but only \(stepProfile.availableSlots ?? 0) are available after reserve. \(durationHint)"
            )
            return nil
        }
        ...
        #if DEBUG
        let availableText = stepProfile.availableSlots.map(String.init) ?? "n/a"
        print("🔎 Playlist planner for \(device.name): context=\(context.rawValue) seam=\(seamMode) legSec=\(String(format: "%.1f", stepProfile.legSeconds)) quality=\(stepProfile.qualityLabel.rawValue) slots=\(stepProfile.slotsRequired) available=\(availableText) reserve=\(stepProfile.reserve.map(String.init) ?? "n/a") fits=\(stepProfile.fitsStorage)")
        ...
        if persist && !hasPresetCapacity(for: device, requiredSlots: requiredSlots, presets: existingPresets, range: persistentAllowedRange) {
            ...
            presentPresetStoreCapacityError(
                device: device,
                detail: "Delete saved transitions, automations, or WLED presets to free preset slots, then try again."
            )
            return nil
        }
        ...
        if resolvedPlaylistId == nil {
            let playlistRange = persistentAllowedRange
            let selectedId = availableFrontmostPlaylistId(excluding: usedPlaylistIds, range: playlistRange)
        ...
        if needsStepIds {
            allocatedPresetIds = availableContiguousIds(
                range: persistentAllowedRange,
        ...
        let presetSaveAttempts = presetSaveRetryAttempts
        let playlistSaveAttempts = playlistSaveRetryAttempts
        ...
        await MainActor.run {
            ...
            DeviceCleanupManager.shared.removeIds(type: .presetStore, deviceId: device.id, ids: [playlistId] + stepPresetIds)
        }
        ...
        var fullRewritePersistentCreateVerified = false
        ...
        for (idx, presetId) in stepPresetIds.enumerated() {
            ...
            let state = segmentedPresetState(
                ...
                compactForPresetStore: true
            )
            let request = WLEDPresetSaveRequest(
                id: presetId,
                name: AesdeticWLEDPresetNameMarker.markedName(
                    "\(stepPrefix)\(presetId)",
                    as: assetKind == .automation ? .automationStep : .transitionStep,
                    ownerId: ownerId ?? (assetKind == .automation ? runId : nil)
                ),
                quickLoad: nil,
                state: state,
        ...
        let markedPlaylistName = AesdeticWLEDPresetNameMarker.markedName(
            playlistName,
            as: assetKind == .automation ? .automation : .savedTransition,
            ownerId: ownerId ?? (assetKind == .automation ? runId : nil)
        )
        let playlistRequest = WLEDPlaylistSaveRequest(
            id: playlistId,
            name: markedPlaylistName,
            ps: stepPresetIds,
            dur: durations,
        ...
        if useFullRewritePersistentCreate {
            do {
                ...
                fullRewritePersistentCreateVerified = true
            } catch {
                #if DEBUG
        ...
        if persist && !fullRewritePersistentCreateVerified {
        ...
        do {
            ...
            #endif
            if case let WLEDAPIError.presetStoreInsufficientSpace(detail) = error {
                presentPresetStoreCapacityError(
                    device: device,
                    detail: "Delete saved transitions, automations, or WLED presets to make room, then try again. \(detail)"
                )
            }
        ...
        applyOptimisticPresetStoreCreate(
            device: device,
            basePresets: existingPresets,
            presetRequests: fullRewritePresetRequests,
    ...
    func saveTransitionPresetToDevice(
        ...
        if let existingPlaylistId = preset.wledPlaylistId,
           existingPlaylistId != result.playlistId,
           !alexaReservedPresetRange.contains(existingPlaylistId) {
            await MainActor.run {
                DeviceCleanupManager.shared.enqueue(
                    type: .playlist,
                    deviceId: device.id,
        ...
        if let existingStepPresetIds = preset.wledStepPresetIds {
            let staleStepIds = Set(existingStepPresetIds)
                .subtracting(Set(result.stepPresetIds))
                .filter { !alexaReservedPresetRange.contains($0) }
    ...
    func saveTransitionPresetWithActiveRunHandling(
        ...
        case .timedOut(let reason):
            #if DEBUG
            print("preset_save.blocked_race_busy device=\(deviceId)\(operationContext) reason=\(reason)")
            #endif
            return .suppressedBusy
        }
        ...
        if isPresetStoreWritePaused(for: deviceId) {
            #if DEBUG
            print("preset_save.blocked device=\(deviceId)\(operationContext) reason=preset_store_paused")
            #endif
            return .suppressedBusy
        }
    ...
    func applyTransitionPreset(
        ...
        await applyTransitionPreset(
            preset,
            to: device,
            queueOnSuppressedBusy: true,
            preferWebSocketFirst: preferWebSocketFirst
        )
    }
    ...
    private func applyTransitionPreset(
        ...
        // Fast path: for synced presets with a stored playlist, replay immediately.
        // Validation reads are intentionally kept off the tap path so a saved
        // transition starts without waiting on presets.json/playlists.json.
        if syncState == .synced,
           ...
           !transitionPresetUsesReservedTempIds(replayPreset) {
            ...
            #endif
            rememberTransitionPresetStartState(replayPreset, to: device)
            ...
            if fastApplied {
                ...
                noteTransitionPresetPlaybackStarted(replayPreset, to: device)
                Task { @MainActor [weak self] in
                    await self?.validateStoredTransitionPresetAfterFastReplay(
                        replayPreset,
                        device: device,
                        playlistId: playlistId
                    )
    ...
    private func validateStoredTransitionPresetAfterFastReplay(
        _ preset: TransitionPreset,
        device: WLEDDevice,
        playlistId: Int
    ) async {
        let validation = await validateStoredTransitionPresetPlaylist(preset, device: device)
        switch validation {
        case .valid, .unknownReadFailure:
            return
        case .legacyPlaylistTiming:
            await markTransitionPresetPendingSync(
                preset,
                error: "Stored playlist uses legacy timing; rebuild required"
            )
            #if DEBUG
            print("transition_preset.apply.fast_replay_validation_legacy_timing playlistId=\(playlistId)")
            #endif
        case .legacyTempRangeIds:
            await markTransitionPresetNeedsMigration(preset)
            #if DEBUG
            print("transition_preset.apply.fast_replay_validation_legacy_temp playlistId=\(playlistId)")
            #endif
        case .missingStepPresets(let ids):
            await markTransitionPresetSyncFailure(preset, error: "Missing step presets: \(ids)")
            #if DEBUG
            print("transition_preset.apply.fast_replay_validation_missing_steps playlistId=\(playlistId) ids=\(ids)")
            #endif
        case .missingPlaylistRecord:
            await markTransitionPresetSyncFailure(preset, error: "Missing playlist record")
            #if DEBUG
            print("transition_preset.apply.fast_replay_validation_missing_playlist playlistId=\(playlistId)")
            #endif
        case .missingPlaylistId:
            await markTransitionPresetSyncFailure(preset, error: "Missing WLED playlist ID")
            #if DEBUG
            print("transition_preset.apply.fast_replay_validation_missing_playlist_id")
            #endif
        }
    }
    ...
    private func runQueuedTransitionPresetApply(deviceId: String, presetId: UUID) async {
        ...
        for _ in 0..<8 {
            ...
            let outcome = await applyTransitionPreset(
                preset,
                to: device,
                queueOnSuppressedBusy: false,
    ...
    private func rebuildTransitionPreset(
        ...
        #endif
        rememberTransitionPresetStartState(preset, to: device)
    ...
    func notePresetStoreDegradedReadable(deviceId: String, message: String) {
        ...
        if previous != .unsafeWritesPaused {
            presentError(
                .apiError(
                    message: "\(message) Automatic saved-item writes are paused. Keep the lamp powered, open its WLED page, and restore a known-good presets.json backup before retrying."
                )
            )
        }
        #if DEBUG
    ...
    func notePresetStoreTransactionStatus(
        deviceId: String,
        status: PresetStoreTransactionStatus,
        message: String? = nil
    ) {
        if status == .idle {
            presetStoreTransactionStatusByDeviceId.removeValue(forKey: deviceId)
        } else {
            presetStoreTransactionStatusByDeviceId[deviceId] = status
        }
        lastPresetStoreHealthEventByDeviceId[deviceId] = Date()
        if let message {
            lastPresetStoreHealthMessageByDeviceId[deviceId] = message
        ...
        case .idle:
            ...
            presetStoreContentHealthByDeviceId[deviceId] = .healthy(hash: nil)
            let automationStore = AutomationStore.shared
            let automationMutationOwnsFinalRefresh =
                automationStore.hasOnDeviceSyncInProgress(for: [deviceId])
                    || automationStore.isDeletionInProgress(for: deviceId)
            if !automationMutationOwnsFinalRefresh,
               let device = devices.first(where: { $0.id == deviceId }) {
                Task { @MainActor in
                    await automationStore.refreshDeviceAutomationState(
                        for: device,
                        reason: .mutationCommitted
                    )
                }
            }
        case .saving, .verifying:
            break
        case .verificationNeeded:
            // This is transport/generation uncertainty, not evidence that
            // presets.json is malformed. Preserve the last content-health fact.
            break
        case .recovering:
            presetStoreHealthByDeviceId[deviceId] = .unsafeWritesPaused
            presetStoreContentHealthByDeviceId[deviceId] = PresetStoreContentHealth(
                state: .confirmedMalformed,
                verifiedHash: nil,
                observedAt: Date(),
                message: message
            )
    ...
    func presetStoreTransactionStatus(for deviceId: String) -> PresetStoreTransactionStatus {
        presetStoreTransactionStatusByDeviceId[deviceId] ?? .idle
    }

    func isPersistentPresetStoreEditBlocked(for deviceId: String) -> Bool {
        presetWriteInProgress.contains(deviceId)
            || (presetStoreTransactionStatusByDeviceId[deviceId]?.blocksPersistentEdits ?? false)
            ...
            || isPresetStoreWritePaused(for: deviceId)
    }

    private func rejectUserPresetStoreMutationIfBusy(
        device: WLEDDevice,
        action: String
    ) async -> Bool {
        guard !(await shouldAllowPresetStoreMutation(deviceId: device.id)) else {
            return false
        }
        presentError(
            .apiError(
                message: "\(device.name) is finishing preset-store work. Please wait before \(action)."
            )
        )
        return true
    }
    ...
    func installProductProfile(
        ...
        if await rejectUserPresetStoreMutationIfBusy(
            device: target,
            action: "installing a profile"
        ) {
            profileInstallErrorByDeviceId[target.id] = "\(target.name) is finishing preset-store work. Please try again shortly."
            return false
        }
        ...
        do {
            ...
            let profileSaveOutcome = try await apiService.savePreset(presetRequest, to: target)
            try requireCommittedPresetStoreOutcome(profileSaveOutcome)
    ...
    private func allocateManagedPresetId(existingPresets: [WLEDPreset], existingManagedPresetIds: [Int]) -> Int {
        ...
        for candidate in existingManagedPresetIds where !used.contains(candidate) {
            if appManagedPresetRange.contains(candidate) {
                return candidate
            }
        }
        ...
        for candidate in appManagedPresetRange where !used.contains(candidate) {
            return candidate
        }
        return 250
    }
    ...
    func cancelActiveRun(
        ...
        await MainActor.run {
            _ = playlistRunsByDevice.remove(device.id)
        }
    ...
    private func nextAppManagedPresetId(excluding existing: Set<Int>) -> Int? {
        for candidate in appManagedPresetRange where !existing.contains(candidate) {
            return candidate
        }
        return nil
    }

    func debugNextPresetIdForTests(existingIds: Set<Int>) -> Int? {
        nextAppManagedPresetId(excluding: existingIds)
    }

    func setAlexaIntegrationEnabled(_ enabled: Bool, for deviceId: String) {
        alexaIntegrationEnabledByDeviceId[deviceId] = enabled
    }

    func isAlexaIntegrationEnabled(for device: WLEDDevice) -> Bool {
        alexaIntegrationEnabledByDeviceId[device.id] ?? false
    }

    func alexaMirrorConflictSlots(for device: WLEDDevice) -> [Int] {
        Array(alexaMirrorConflictSlotsByDeviceId[device.id] ?? []).sorted()
    }

    func clearAlexaMirrorConflicts(for device: WLEDDevice) {
        alexaMirrorConflictSlotsByDeviceId.removeValue(forKey: device.id)
        PresetsStore.shared.markAlexaFavoritesForDevice(device.id, state: .pending)
    }

    func alexaFavoriteCandidates(for device: WLEDDevice) -> [AlexaFavoriteCandidate] {
        let store = PresetsStore.shared
        var candidates: [AlexaFavoriteCandidate] = []
        candidates.append(contentsOf: store.colorPresets.compactMap { preset in
            guard let wledId = preset.wledPresetIds?[device.id] ?? preset.wledPresetId,
                  appManagedPresetRange.contains(wledId) else { return nil }
            return AlexaFavoriteCandidate(
                sourceType: .color,
                sourceId: preset.id,
                sourceWLEDPresetId: wledId,
                displayName: preset.name
            )
        })
        candidates.append(contentsOf: store.effectPresets(for: device.id).compactMap { preset in
            guard let wledId = preset.wledPresetId,
                  appManagedPresetRange.contains(wledId) else { return nil }
            return AlexaFavoriteCandidate(
                sourceType: .effect,
                sourceId: preset.id,
        ...
        candidates.append(contentsOf: store.transitionPresets(for: device.id).compactMap { preset in
    ...
    func alexaFavoriteCandidate(
        sourceType: AlexaFavoriteSourceType,
        sourceId: UUID?,
        wledPresetId: Int,
        name: String
    ) -> AlexaFavoriteCandidate? {
        guard appManagedPresetRange.contains(wledPresetId) else { return nil }
        return AlexaFavoriteCandidate(
            sourceType: sourceType,
            sourceId: sourceId,
            sourceWLEDPresetId: wledPresetId,
            displayName: name
        )
    }

    @discardableResult
    func addAlexaFavorite(_ candidate: AlexaFavoriteCandidate, for device: WLEDDevice) -> Bool {
        let added = PresetsStore.shared.addAlexaFavorite(candidate, for: device.id)
        if added {
            alexaMirrorConflictSlotsByDeviceId.removeValue(forKey: device.id)
    ...
    func removeAlexaFavorite(_ favorite: AlexaFavorite, for device: WLEDDevice) {
        PresetsStore.shared.removeAlexaFavorite(favorite.id, for: device.id)
        SmartHomeIntegrationStore.shared.setStatus(
            .needsSync,
            for: .alexa,
            deviceId: device.id
    ...
    func syncAlexaFavoritesToDevice(
        _ device: WLEDDevice,
        enabled: Bool,
        invocationName: String,
        allowReplacingExisting: Bool = false
    ) async -> Bool {
        guard !alexaMirrorSyncInProgressByDeviceId.contains(device.id) else { return false }
        alexaMirrorSyncInProgressByDeviceId.insert(device.id)
        defer {
            alexaMirrorSyncInProgressByDeviceId.remove(device.id)
        }

        let trimmedName = invocationName.trimmingCharacters(in: .whitespacesAndNewlines)
        let resolvedName = trimmedName.isEmpty ? device.name : trimmedName

        do {
            let currentSettings = try await WLEDAPIService.shared.fetchAlexaIntegrationSettings(for: device)
            guard currentSettings.isSupported else {
                setAlexaIntegrationEnabled(false, for: device.id)
                SmartHomeIntegrationStore.shared.setStatus(
                    .unsupported,
                    for: .alexa,
                    deviceId: device.id,
                    message: "This WLED firmware build does not include Alexa support.",
            ...
            if !enabled {
                let settings = WLEDAlexaIntegrationSettings(
                    isEnabled: false,
                    invocationName: resolvedName,
                    exposedPresetCount: 0
                )
                try await WLEDAPIService.shared.updateAlexaIntegrationSettings(settings, for: device)
                setAlexaIntegrationEnabled(false, for: device.id)
                SmartHomeIntegrationStore.shared.setStatus(
                    .notSetUp,
                    for: .alexa,
                    deviceId: device.id,
                    message: "Alexa control is off for this WLED device.",
                    verificationSource: .reportedByDevice
                )
                return true
            }
            ...
            let store = PresetsStore.shared
            if store.alexaAutoFillEnabled(for: device.id) {
                _ = store.autoFillAlexaFavorites(for: device.id, candidates: alexaFavoriteCandidates(for: device))
            }

            let favorites = store.alexaFavorites(for: device.id)
            store.markAlexaFavorites(favorites, state: .pending)
            let mirrors = favorites.map {
                WLEDAlexaMirrorFavorite(
                    slot: $0.slot,
                    sourcePresetId: $0.sourceWLEDPresetId,
                    displayName: $0.displayName
                )
            }
            let result = try await WLEDAPIService.shared.syncAlexaPresetMirrors(
                favorites: mirrors,
                device: device,
                allowReplacingExisting: allowReplacingExisting
            )

            guard result.conflictSlots.isEmpty else {
                alexaMirrorConflictSlotsByDeviceId[device.id] = Set(result.conflictSlots)
                store.markAlexaFavorites(favorites, state: .conflict, error: "Alexa slots \(result.conflictSlots.map(String.init).joined(separator: ", ")) already contain WLED presets.")
                SmartHomeIntegrationStore.shared.setStatus(
                    .conflict,
                    for: .alexa,
                    deviceId: device.id,
                    message: "Alexa slots \(result.conflictSlots.map(String.init).joined(separator: ", ")) already contain WLED presets.",
                    verificationSource: .reportedByDevice
                )
                return false
            }
            guard result.missingSourceIds.isEmpty else {
                store.markAlexaFavorites(favorites, state: .failed, error: "Source presets missing: \(result.missingSourceIds.map(String.init).joined(separator: ", "))")
                SmartHomeIntegrationStore.shared.setStatus(
                    .failed,
                    for: .alexa,
                    deviceId: device.id,
                    message: "Source presets missing: \(result.missingSourceIds.map(String.init).joined(separator: ", "))"
                )
                return false
            }
            ...
            let settings = WLEDAlexaIntegrationSettings(
                isEnabled: true,
                invocationName: resolvedName,
                exposedPresetCount: min(alexaReservedPresetRange.count, favorites.count)
            )
            try await WLEDAPIService.shared.updateAlexaIntegrationSettings(settings, for: device)
            alexaMirrorConflictSlotsByDeviceId.removeValue(forKey: device.id)
            setAlexaIntegrationEnabled(true, for: device.id)
            store.markAlexaFavorites(favorites, state: .synced)
            SmartHomeIntegrationStore.shared.setStatus(
                .enabled,
                for: .alexa,
                deviceId: device.id,
            ...
            let favorites = PresetsStore.shared.alexaFavorites(for: device.id)
            PresetsStore.shared.markAlexaFavorites(favorites, state: .failed, error: error.localizedDescription)
            SmartHomeIntegrationStore.shared.setStatus(
                .failed,
                for: .alexa,
                deviceId: device.id,
    ...
    private func applyOptimisticPresetStoreCreate(
        device: WLEDDevice,
        basePresets: [WLEDPreset],
        presetRequests: [WLEDPresetSaveRequest],
        playlistRequest: WLEDPlaylistSaveRequest
    ) {
        var presetsById = Dictionary(uniqueKeysWithValues: basePresets.map { ($0.id, $0) })
        for request in presetRequests {
            presetsById[request.id] = WLEDPreset(
                id: request.id,
                name: request.name,
                quickLoad: request.quickLoad,
                segment: nil,
                state: request.state
            )
        }

        let mergedPresets = presetsById.values.sorted { $0.id < $1.id }
        presetsCache[device.id] = mergedPresets
        recordPresetNameMap(for: device.id, presets: mergedPresets)
        updatePresetSlotStatus(for: device, presets: mergedPresets)

        var playlists = playlistsCache[device.id] ?? []
        let playlist = WLEDPlaylist(
            id: playlistRequest.id,
            name: playlistRequest.name,
            presets: playlistRequest.ps,
            duration: playlistRequest.dur,
            transition: playlistRequest.transition,
            repeat: playlistRequest.repeat,
            endPresetId: playlistRequest.endPresetId,
            shuffle: playlistRequest.shuffle
        )
        ...
        playlists.sort { $0.id < $1.id }
        playlistsCache[device.id] = playlists
        recordPlaylistNameMap(for: device.id, playlists: playlists)
    }

    private func applyOptimisticPresetStoreDelete(
        deviceId: String,
        playlistIds: [Int],
        presetIds: [Int]
    ) {
        ...
        if !presetIdSet.isEmpty, let presets = presetsCache[deviceId] {
            let updatedPresets = presets.filter { !presetIdSet.contains($0.id) }
            presetsCache[deviceId] = updatedPresets
            recordPresetNameMap(for: deviceId, presets: updatedPresets)
            if let device = devices.first(where: { $0.id == deviceId }) {
                updatePresetSlotStatus(for: device, presets: updatedPresets)
            }
        }

        if !playlistIdSet.isEmpty, let playlists = playlistsCache[deviceId] {
            let updatedPlaylists = playlists.filter { !playlistIdSet.contains($0.id) }
            playlistsCache[deviceId] = updatedPlaylists
            recordPlaylistNameMap(for: deviceId, playlists: updatedPlaylists)
        }
    }

    private func recordPresetNameMap(for deviceId: String, presets: [WLEDPreset]) {
        let map = Dictionary(uniqueKeysWithValues: presets.map { ($0.id, $0.displayName) })
    ...
    private func recordPlaylistNameMap(for deviceId: String, playlists: [WLEDPlaylist]) {
        let map = Dictionary(uniqueKeysWithValues: playlists.map { ($0.id, $0.displayName) })
    ...
    private func retryPendingPlaylistRenames(for device: WLEDDevice) async -> Bool {
        ...
        for entry in pending {
            do {
                let outcome = try await apiService.renamePlaylistRecord(
                    id: entry.playlistId,
                    name: entry.desiredName,
                    device: device
                )
                try requireCommittedPresetStoreOutcome(outcome)
    ...
    func deletePresetRecord(_ presetId: Int, for device: WLEDDevice) async -> Bool {
        ...
        do {
            ...
            applyOptimisticPresetStoreDelete(deviceId: device.id, playlistIds: [], presetIds: [presetId])
            await refreshPresets(for: device)
            await refreshPlaylists(for: device)
            clearError()
            return true
    ...
    private func rewritePresetStoreDeletingRecordsWithJournal(
        ...
        let cleanupTargets = try await apiService.capturePresetStoreCleanupTargets(
    ...
    func deleteColorPreset(_ preset: ColorPreset, for device: WLEDDevice) async -> Bool {
        guard !AutomationStore.shared.isAutomationDeletionInProgress(for: device.id) else {
    ...
    func deletePlaylist(_ playlist: WLEDPlaylist, for device: WLEDDevice) async -> Bool {
        ...
        do {
            ...
            applyOptimisticPresetStoreDelete(deviceId: device.id, playlistIds: [playlist.id], presetIds: [])
            await refreshPlaylists(for: device)
            await refreshPresets(for: device)
            clearError()
            return true
    ...
    func renamePresetRecord(_ presetId: Int, to name: String, for device: WLEDDevice) async -> Bool {
        ...
        if await rejectUserPresetStoreMutationIfBusy(
            device: device,
            action: "renaming a preset"
        ) {
            return false
        }
        markUserInteraction(device.id)
        do {
            let outcome = try await apiService.renamePresetRecord(
                id: presetId,
                name: name,
                device: device
            )
    ...
    func renamePlaylistRecord(_ playlistId: Int, to name: String, for device: WLEDDevice) async -> Bool {
        ...
        if deferForActiveRun {
            presentError(.apiError(message: "Stop the active run before renaming this playlist."))
            #if DEBUG
            print("playlist.rename.blocked device=\(device.id) playlistId=\(playlistId) reason=active_run")
            #endif
            return false
        }
        do {
    ...
    func nextPlaylistId(for device: WLEDDevice) -> Int? {
        ...
        for id in appManagedPresetRange where !usedIds.contains(id) {
            return id
        }
        return nil
    }
    ...
    func deleteTransitionPreset(_ preset: TransitionPreset, for device: WLEDDevice) async -> Bool {
        ...
        do {
            ...
            applyOptimisticPresetStoreDelete(deviceId: device.id, playlistIds: [resolvedPlaylistId], presetIds: stepPresetIds)
            await refreshPlaylists(for: device)
            await refreshPresets(for: device)
            clearError()
            #if DEBUG
    ...
    func deleteEffectPreset(_ preset: WLEDEffectPreset, for device: WLEDDevice) async -> Bool {
        ...
        do {
            ...
            applyOptimisticPresetStoreDelete(deviceId: device.id, playlistIds: [], presetIds: [resolvedPresetId])
            await refreshPresets(for: device)
            await refreshPlaylists(for: device)
            clearError()
            #if DEBUG
    ...
    private func presetStoreNamesMatch(_ lhs: String, _ rhs: String) -> Bool {
        AesdeticWLEDPresetNameMarker.displayName(from: lhs) == AesdeticWLEDPresetNameMarker.displayName(from: rhs)
    }
    ...
    func savePreset(name: String, quickLoadTag: String? = nil, for device: WLEDDevice, presetId: Int? = nil) async {
        ...
        do {
            ...
            guard let targetId = presetId ?? nextPresetId(for: device) else {
                updatePresetSlotStatus(for: device)
                presentError(.apiError(message: "No available app preset slots. Free up presets before saving a new one."))
                presetLoadingStates[device.id] = false
                return
            }
            ...
            let outcome = try await apiService.savePreset(request, to: device)
            try requireCommittedPresetStoreOutcome(outcome)
            DeviceCleanupManager.shared.removeIds(type: .presetStore, deviceId: device.id, ids: [targetId])
            let presets = try await apiService.fetchPresets(for: device)
            presetsCache[device.id] = presets
            recordPresetNameMap(for: device.id, presets: presets)
            updatePresetSlotStatus(for: device, presets: presets)
    ...
    @discardableResult
    private func rememberMainGradient(_ gradient: LEDGradient, for deviceId: String) -> [GradientStop] {
        let sortedStops = gradient.stops.sorted { $0.position < $1.position }
        latestGradientStops[deviceId] = sortedStops
        persistLatestGradient(sortedStops, for: deviceId)
        markGradientHydratedFromLiveState(deviceId)
        gradientApplicationTimes[deviceId] = Date()
        return sortedStops
    }

    private func updateLocalDeviceAppearance(
        deviceId: String,
        gradient: LEDGradient,
        brightness: Int?,
        isOn: Bool? = true
        ...
        if !sortedStops.isEmpty {
            updatedDevice.currentColor = GradientSampler.sampleColor(
                at: 0.5,
                stops: sortedStops,
                interpolation: gradient.interpolation
            )
            updatedDevice.temperature = nil
        }
        if let brightness {
            updatedDevice.brightness = brightness
            if brightness > 0 {
                lastBrightnessBeforeOff[deviceId] = brightness
            }
        }
        if let isOn {
            updatedDevice.isOn = isOn
        } else if let brightness {
            updatedDevice.isOn = brightness > 0
        }
        updatedDevice.isOnline = true
        updatedDevice.lastSeen = Date()
        devices[index] = updatedDevice
    ...
    func noteColorPresetApplied(_ preset: ColorPreset, to device: WLEDDevice) async {
        let gradient = LEDGradient(
            stops: preset.gradientStops,
            interpolation: preset.gradientInterpolation ?? .linear
        )
        let brightness = (preset.includeBrightness == false) ? nil : preset.brightness
        rememberMainGradient(gradient, for: device.id)
        updateLocalDeviceAppearance(
            deviceId: device.id,
            gradient: gradient,
            brightness: brightness,
            isOn: brightness.map { $0 > 0 } ?? true
        )
        var segmentStates = effectStates[device.id] ?? [:]
        var state = segmentStates[0] ?? .default
        state.effectId = 0
        state.isEnabled = false
        segmentStates[0] = state
        effectStates[device.id] = segmentStates
        clearError()
    }

    func noteEffectPresetApplied(_ preset: WLEDEffectPreset, to device: WLEDDevice) async {
        if let stops = preset.gradientStops, !stops.isEmpty {
            ...
            devices[index].brightness = preset.brightness
            devices[index].isOn = preset.brightness > 0
            devices[index].isOnline = true
            devices[index].lastSeen = Date()
        }

        var state = currentEffectState(for: device, segmentId: 0)
        state.effectId = preset.effectId
        state.speed = preset.speed ?? state.speed
        state.intensity = preset.intensity ?? state.intensity
        state.paletteId = preset.paletteId
        state.isEnabled = preset.effectId != 0
        updateEffectStateCache(state, deviceId: device.id, segmentId: 0)
        clearError()
    }

    private func rememberTransitionPresetStartState(_ preset: TransitionPreset, to device: WLEDDevice) {
        rememberMainGradient(preset.gradientA, for: device.id)
        updateLocalDeviceAppearance(
            deviceId: device.id,
            gradient: preset.gradientA,
            brightness: preset.brightnessA,
            isOn: preset.brightnessA > 0
        )
    }

    private func noteTransitionPresetPlaybackStarted(_ preset: TransitionPreset, to device: WLEDDevice) {
        rememberTransitionPresetStartState(preset, to: device)
        scheduleTransitionPresetEndState(preset, deviceId: device.id, runId: activeRunStatus[device.id]?.id)
    }

    private func scheduleTransitionPresetEndState(
        _ preset: TransitionPreset,
        deviceId: String,
        runId: UUID?
    ) {
        guard let runId else { return }
        let duration = max(0.1, preset.durationSec)
        let delay = UInt64((duration * 1_000_000_000).rounded())
        Task { [weak self] in
            try? await Task.sleep(nanoseconds: delay)
            self?.rememberTransitionPresetEndStateIfCurrent(preset, deviceId: deviceId, runId: runId)
        }
    }

    private func rememberTransitionPresetEndStateIfCurrent(
        _ preset: TransitionPreset,
        deviceId: String,
        runId: UUID
    ) {
        guard activeRunStatus[deviceId]?.id == runId else { return }
        rememberMainGradient(preset.gradientB, for: deviceId)
        updateLocalDeviceAppearance(
            deviceId: deviceId,
            gradient: preset.gradientB,
            brightness: preset.brightnessB,
            isOn: preset.brightnessB > 0
        )
    }
...
extension DeviceControlViewModel {
    ...
    private func updatePresetSlotStatus(for device: WLEDDevice, presets: [WLEDPreset]? = nil) {
        ...
        let total = appManagedPresetRange.count
